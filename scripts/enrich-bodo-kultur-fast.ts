/**
 * Rask Bodø kultur-enrichment: Brreg roller → Brreg kontakt → 1881/Gulesider → DDG Maps.
 * Maks 25 minutter. E-post hoppes ALDRI over.
 *
 * Kjør: npx tsx scripts/enrich-bodo-kultur-fast.ts
 *      npx tsx scripts/enrich-bodo-kultur-fast.ts --phase owners|brreg|contacts|maps
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";
import { createServiceClient } from "../src/lib/supabase/service.ts";
import { CompanyPatchBuffer } from "./lib/enrich-batch-db.ts";
...
async function refreshContactFromBrreg(
  ...
  if (notes.length > 0 && !dryRun) {
    await buffer.queue(company.orgnr, patch);
  }
...
async function phaseContacts(companies: Company[], progress: Progress, dryRun: boolean) {
  ...
  const { stopped } = await mapPool(targets, CONCURRENCY_CONTACTS, async (company) => {
    try {
      ...
      if (needsPhone && !phone && mapsDiscovery?.phone?.trim()) {
        phone = mapsDiscovery.phone.trim();
        source = "DDG Maps";
      }
      ...
      if (needsPhone && !progress.maps.includes(company.orgnr)) {
...
async function main() {
  ...
  console.log(`Tidsbudsjett: 25 min · Fase: ${phase}`);
