/**
 * Rask Bodø reklame og markedsføring-enrichment: Brreg roller → Brreg kontakt → 1881/Gulesider → DDG Maps.
 * Ingen full nettside-scan. E-post hoppes ALDRI over.
 *
 * Kjør: npx tsx scripts/enrich-bodo-reklame-fast.ts
 *      npx tsx scripts/enrich-bodo-reklame-fast.ts --phase owners|brreg|contacts|maps
 */
...
function migrateShardProgress(companies: Company[], progress: Progress): number {
  ...
  const shardFiles = readdirSync(cacheDir).filter((f) =>
    /^bodo-reklame-enrich-progress-shard-\d+\.json$/.test(f)
  );
