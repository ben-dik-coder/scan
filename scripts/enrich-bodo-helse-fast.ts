/**
 * Rask Bodø helse og omsorg-enrichment: Brreg roller → Brreg kontakt → 1881/Gulesider → DDG Maps.
 * Ingen full nettside-scan. E-post hoppes ALDRI over.
 *
 * Kjør: npx tsx scripts/enrich-bodo-helse-fast.ts
 *      npx tsx scripts/enrich-bodo-helse-fast.ts --phase owners|brreg|contacts|maps
 */
...
const PROGRESS_FILE = resolve(process.cwd(), "scripts/.cache/bodo-helse-fast-progress.json");
...
function migrateShardProgress(companies: Company[], progress: Progress): number {
  ...
  const shardFiles = readdirSync(cacheDir).filter((f) =>
    /^bodo-helse-enrich-progress-shard-\d+\.json$/.test(f)
  );
...
async function refreshContactFromBrreg(
  ...
  if (dryRun) {
    ...
    if (brregPhone && !hadPhone) {
      addedPhone = true;
      notes.push(`telefon ${brregPhone} (Brreg)`);
    }
    if (brregEmail && !hadEmail) {
      addedEmail = true;
  ...
  const notes: string[] = [];
  if (addedPhone) {
    notes.push(`telefon ${(updated.mobile ?? updated.phone ?? "").trim()} (Brreg)`);
  }
  if (addedEmail) {
    notes.push(`e-post ${updated.email} (Brreg)`);
  }
